...
[![Build Status](https://github.com/easyploy/php/workflows/Build%20docker%20image/badge.svg)](https://github.com/easyploy/php/actions)
[![Docker Pulls](https://img.shields.io/docker/pulls/easyploy/php.svg)](https://hub.docker.com/r/easyploy/php)
[![Docker Stars](https://img.shields.io/docker/stars/easyploy/php.svg)](https://hub.docker.com/r/easyploy/php)
[![Docker Layers](https://images.microbadger.com/badges/image/easyploy/php.svg)](https://microbadger.com/images/easyploy/php)
...
- [Images based on `easyploy/php`](#images-based-on-easyployphp)
- [Orchestration Actions](#orchestration-actions)

## Docker Images

❗For better reliability we release images with stability tags (`easyploy/php:8.0-X.X.X`) which correspond to [git tags](https://github.com/easyploy/php/releases). We strongly recommend using images only with stability tags. 

About images:

- All images based on Alpine Linux
- Base image: [php](https://github.com/docker-library/php)
- [GitHub actions builds](https://github.com/easyploy/php/actions) 
- [Docker Hub](https://hub.docker.com/r/easyploy/php) 
...
- `sudo` allowed for all commands for `easyploy` user
...
Same as `-dev` but the default user/group `easyploy` has uid/gid `501`/`20`  to match the macOS default user/group ids.
...
| `PHP_VER`        |               |
| `PHP_DEV`        |               |
| `EASYPLOY_GROUP_ID` | `1000`        |
| `EASYPLOY_USER_ID`  | `1000`        |

Change `EASYPLOY_USER_ID` and `EASYPLOY_GROUP_ID` mainly for local dev version of images, if it matches with existing system user/group ids the latter will be deleted. 
...
Changes per stability tag reflected in git tags description under [releases](https://github.com/easyploy/python/releases). 
...
You can run SSHD with this image by changing the command to `sudo /usr/sbin/sshd -De` and mounting authorized public keys to `/home/easyploy/.ssh/authorized_keys`

## Adding SSH key

You can add a private SSH key to the container by mounting it to `/home/easyploy/.ssh/id_rsa`

## Users and permissions

Default container user is `easyploy:easyploy` (UID/GID `1000`). PHP-FPM runs from `www-data:www-data` user (UID/GID `82`) by default. User `easyploy` is a part of `www-data` group.

Codebase volume `$APP_ROOT` (`/var/www/html`) owned by `easyploy:easyploy`. Files volume `$FILES_DIR` (`/mnt/files`) owned by `www-data:www-data` with `775` mode.

#### Helper scripts 

- `files_chmod` – in case you need write access for `easyploy` user to a file/dir generated by `www-data` on this volume run `sudo files_chmod [FILEPATH]` script (FILEPATH must be under `/mnt/files`), it will recursively change the mode to `ug=rwX,o=rX`

- `files_chown` – in case you manually uploaded files under `easyploy` user to files volume and want to change the ownership of those files to `www-data` run `sudo files_chown [FILEPATH]` script (FILEPATH must be under `/mnt/files`), it will recursively change ownership to `www-data:www-data`
...
[_(8/Dockerfile)_]: https://github.com/easyploy/php/tree/master/8/Dockerfile
[_(7/Dockerfile)_]: https://github.com/easyploy/php/tree/master/7/Dockerfile

[7.x xdebug]: https://github.com/easyploy/php/tree/master/7/templates/docker-php-ext-xdebug.ini.tmpl
[8.x xdebug]: https://github.com/easyploy/php/tree/master/8/templates/docker-php-ext-xdebug.ini.tmpl
[7.x pcov]: https://github.com/easyploy/php/tree/master/7/templates/docker-php-ext-pcov.ini.tmpl
[8.x pcov]: https://github.com/easyploy/php/tree/master/8/templates/docker-php-ext-pcov.ini.tmpl
[7.x sqlsrv]: https://github.com/easyploy/php/tree/master/7/templates/docker-php-ext-sqlsrv.ini.tmpl
[7.x newrelic]: https://github.com/easyploy/php/tree/master/7/templates/docker-php-ext-newrelic.ini.tmpl
[8.0 session]: https://github.com/easyploy/php/tree/master/8/templates/docker-php-8.0.ini.tmpl
[7.4 session]: https://github.com/easyploy/php/tree/master/7/templates/docker-php-7.4.ini.tmpl
[7.3 session]: https://github.com/easyploy/php/tree/master/7/templates/docker-php-7.3.ini.tmpl
...
[latest]: https://github.com/easyploy/pecl-php-uploadprogress/releases/tag/latest
